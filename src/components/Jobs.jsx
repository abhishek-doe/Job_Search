...
const Jobs = () => {
  ...
  const jobElement =
    ...
    jobData.map((job) => {
      ...
      return (
        <div className="group w-[350px] p-[20px] bg-white rounded-[10px] duration-500 hover:bg-[#4682b4] shadow-lg shadow-greyIsh-400/700 hover:shadow-lg" key={job.id} onClick={() => console.log(job.id)}>
...
console.log(jobDt)
  return (
    <div>
      <div className="jobContainer grid grid-cols-3 2xl:grid-cols-2 xl:grid-cols-1 gap-10 justify-center flex-wrap items-center py-6">{jobElement}</div>
    </div>
  );
};
